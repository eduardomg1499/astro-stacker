...
// #[tauri::command]
async fn generate_smart_ap_grid_legacy(
    app: tauri::AppHandle,
    ...
    // Default to frame 0 for analysis if not specified.
    let raw = r.get_frame(0, cid);
    if raw.is_empty() {
    ...
    // Rows
    for y in (step..h.saturating_sub(step)).step_by(step) {
        // Cols
        for x in (step..w.saturating_sub(step)).step_by(step) {
            // 3. Calculate Variance / Contrast in this Box
            let start_x = x.saturating_sub(half_step);
            let start_y = y.saturating_sub(half_step);
            let end_x = (x + half_step).min(w);
            let end_y = (y + half_step).min(h);
            ...
            for cy in start_y..end_y {
                ...
                for cx in start_x..end_x {
                    // Handle Color/Mono. If color, take Green or Mono equiv
                    let val = if u16_data.len() == w * h {
                        u16_data[row_off + cx]
                    } else {
                        // RGB: Take Green (Index 1)
                        if (row_off + cx) * 3 + 1 < u16_data.len() {
                            u16_data[(row_off + cx) * 3 + 1]
                        } else {
                            0
                        }
                    };
            ...
            // Normalized Contrast (0.0 - 1.0)
            let delta = (max_val as f32 - min_val as f32) / 65535.0;
    ...
    // Always add at least one center point if none found (Safety)
    if points.is_empty() {
        points.push(ApPoint {
            x: (w / 2) as f32,
            y: (h / 2) as f32,
